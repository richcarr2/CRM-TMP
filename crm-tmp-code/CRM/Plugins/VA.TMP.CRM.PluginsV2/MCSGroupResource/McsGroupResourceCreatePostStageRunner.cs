...
namespace VA.TMP.CRM
{
    ...
    public class McsGroupResourceCreatePostStageRunner : PluginRunner
    {
        ...
        public override void Execute()
        {
            ...
            UpdatePSWarning(thisGR, relatedRG);
        ...
        internal void UpdatePSWarning(mcs_groupresource thisGR, Guid relatedRG)
        {
            var vistaClinics = new List<cvt_schedulingresource>();

            if (relatedRG.Equals(Guid.Empty))
            {
                Logger.WriteDebugMessage("Missing Related Resource Group");
                return;
            }
            ...
            try
            {
                ...
                {
                    var schedulingResource = srv.cvt_schedulingresourceSet.FirstOrDefault(sr => sr.cvt_tmpresourcegroup.Id == relatedRG);
                    ...
                    if (participatingSite == null || participatingSite.Id.Equals(Guid.Empty)) return;

                    var canBeScheduled = participatingSite.Contains("cvt_scheduleable") ? participatingSite.cvt_scheduleable : null;

                    //Logger.WriteDebugMessage($"Resource Group Type: {resourceGroup.mcs_Type.Value}");
                    Logger.WriteDebugMessage($"Group Resource Type: {thisGR.mcs_Type.Value}");

                    var resource = srv.mcs_resourceSet.FirstOrDefault(r => r.Id == thisGR.mcs_RelatedResourceId.Id);
        ...
        internal void UpdateTSSResourceGroup(mcs_groupresource thisGroupResource)
        {
            ...
            Logger.WriteDebugMessage("starting UpdateTSSResourceGroup - RC");
            ...
            try
            {
                ...
                Logger.WriteDebugMessage("thisTSSResourceGroupId - RC:" +thisTSSResourceGroupId);
                ...
                using (var srv = new Xrm(OrganizationService))
                {
                    ...
                    switch (thisTSSResourceGroup.mcs_Type.Value)
                    {
                        ...
                        case (int)mcs_resourcetype.PairedResourceGroup:
                            Logger.WriteDebugMessage("TSS Resource Group is Paired. RC ");
                            foreach (var mcsResource in getEquipmentResources)
                            {
                                builderCBG = buildFunction(mcsResource.mcs_relatedResourceId, resourceString, builderCBG, count, out count, out resourceString);
                            }
                ...
                Logger.WriteDebugMessage("Constraints XML Constructed - RC");

                if (thisTSSResourceGroup.Contains("mcs_relatedresourcegroupid"))
                {
                    Logger.WriteDebugMessage("mcs_relatedresourcegroupid exists");
                }
                else
                {
                    Logger.WriteDebugMessage("mcs_relatedresourcegroupid does NOT exist");
                }
                ...
                Logger.WriteDebugMessage("see if name changed");
