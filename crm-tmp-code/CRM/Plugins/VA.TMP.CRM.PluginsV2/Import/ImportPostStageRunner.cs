...
{
    public class ImportPostStageRunner : PluginRunner
    {
        public ImportPostStageRunner(IServiceProvider serviceProvider) : base(serviceProvider) { }
        ...
        public override void Execute()
        {
            ...
            if (PrimaryEntity.Attributes.Contains("tmp_name"))
            {
                if (PrimaryEntity.Attributes["tmp_name"].ToString().Contains("- Successfully completed at"))
                {
                    //Logger.WriteDebugMessage("Successful, so don't run any update code again.");
                    return;
                }

                //if (PrimaryEntity.Attributes["tmp_name"].ToString().Contains("IENMatching"))
                //    ImportVistAClinicIENs();

                if (PrimaryEntity.Attributes["tmp_name"].ToString().ToLower().StartsWith("imdataimport"))
                            ImportIMRecords();
            }
        }
        ...
        private void ImportIMRecords()
        {
            ...
            using (var srv = new Xrm(OrganizationService))
            {
                ...
                foreach (DataRow rt in distResourcesDT.Rows)
                {
                    ...
                    if (stgRes == null)
                        ProcessIMRecords(result, srv, bulkAddUpdate);
                }
                ...
                UpdateStagingResourceOwnership(srv);
                DeactivateSettingsRecord();
