...
namespace VA.TMP.CRM
{
    public class ResourcePackageCreatePreStageRunner : PluginRunner
    {
        ...
        public override void Execute()
        {
            ...
            {
                //Logger.WriteDebugMessage(String.Format("The Resource Package name should be different than {0}, updating it in the CreatePreStage to: {1}.", name, derivedName));
        ...
        {            
            ...
            using (var srv = new Xrm(OrganizationService))
            {
                ...
                var facilityName = srv.mcs_facilitySet.Where(f => f.mcs_facilityId.Value == thisRecord.cvt_providerfacility.Id).FirstOrDefault().mcs_name;                
                ...
                if (provFacility != null && provFacility != Guid.Empty)
                {
                    ...
                    var teamCheck = checkTeam(provFTCTeam, "Provider FTC", facilityName);
                    teamCheck += checkTeam(provSCTeam, "Provider Service Chief", facilityName);
                    teamCheck += checkTeam(provCOSTeam, "Provider Chief of Staff", facilityName);
                    
                    teamCheck = AppendEndMessage(teamCheck, facilityName);
                ...
                if (thisRecord.cvt_groupappointment.HasValue && thisRecord.cvt_groupappointment.Value == true)
                {
                    if (thisRecord.cvt_intraorinterfacility != null && thisRecord.cvt_intraorinterfacility.Value == (int)cvt_resourcepackagecvt_intraorinterfacility.Interfacility)
                    {
                        ...
                        if (patFacility != null && patFacility != Guid.Empty)
                        {
                            ...
                            //Check if team exists and if there are team members.
                            var teamCheck = checkTeam(patFTCTeam, "Patient FTC", facilityName);
                            teamCheck += checkTeam(patSCTeam, "Patient Service Chief", facilityName);
                            teamCheck += checkTeam(patCOSTeam, "Patient Chief of Staff", facilityName);
                ...
                if (hub != null && hub != Guid.Empty)
                {
                    ...
                    //Check if team exists and if there are team members.
                    var teamCheck = checkTeam(hubDirectorTeam, "Hub Director", facilityName);
                    teamCheck += checkTeam(hubTSAManagerTeam, "Hub TSA Manager", facilityName);
        ...
        public string checkTeam(Team thisTeam, string check, string facilityName = null)
        {
            var message = "";

            if (thisTeam == null)
            {
                message = $"The {check} team is missing at {facilityName}. ";
            }
            else
            {
                var facilityDetails = thisTeam.Name.Split('@');
                ...
                using (var srv = new Xrm(OrganizationService))
                {
                    ...
                    {
                        message = $"The {facilityDetails[0]} team at {facilityDetails[1]} has no members.  ";
                    }
                }
            }
            return message;
        ...
        private string AppendEndMessage(string message, string facilityName)
        {
            if (!string.IsNullOrEmpty(message))
            {
                if (message.Contains("team is missing at") && message.Contains("has no members"))
                {
                    return message += $"   To fix this issue, click the {facilityName} link, select Related and then Teams to create the team or add team members.";
                }
                else if (message.Contains("team is missing at") && !message.Contains("has no members"))
                {
                    return message += $"   To fix this issue, click the {facilityName} link, select Related and then Teams to create the  team.";
                }
                else if (!message.Contains("team is missing at") && message.Contains("has no members"))
                {
                    return message += $"   To fix this issue, click the {facilityName} link, select Related and then Teams to add team members.";
                }
            }
            return string.Empty;
        }
