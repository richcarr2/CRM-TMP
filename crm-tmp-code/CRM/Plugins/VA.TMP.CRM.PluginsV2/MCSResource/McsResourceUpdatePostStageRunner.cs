...
namespace VA.TMP.CRM
{
    public class McsResourceUpdatePostStageRunner : PluginRunner
    {
        ...
        internal void UpdateSystemResource(Guid thisId, mcs_resource resource)
        {
            ...
            Logger.WriteGranularTimingMessage("Starting UpdateSystemResource - RC");
            using (var srv = new Xrm(OrganizationService))
            {
                try
                {                  
                    //Grabs all the attributes needed to update the associated System Resource of the mcs_Resource that was just updated. 
                    if (!resource.Contains("mcs_relatedresourceid"))
                    {
                        Logger.WriteGranularTimingMessage("no  mcs_relatedresourceid");
                        return;
                    }
                    Logger.WriteGranularTimingMessage("Found mcs_relatedresourceid - RC");
                    ...
                    Logger.WriteGranularTimingMessage("Found equipment");
                    ...
                    else
                    {
                        ...
                        Logger.WriteGranularTimingMessage("CvtHelper.UpdateField");
                    }

                     var equipSite = srv.mcs_siteSet.FirstOrDefault(i => i.Id == McsHelper.getEntRefID("mcs_relatedsiteid"));
                    if (equipSite == null)
                        return;
                    if (equipSite.Contains("mcs_relatedactualsiteid"))
                    {
                        Logger.WriteGranularTimingMessage("Found mcs_RelatedActualSiteId");
                        Logger.WriteDebugMessage("got mcssite: " + equipSite.mcs_RelatedActualSiteId.Name);
                    }
                    Logger.WriteGranularTimingMessage("Found mcssite");
