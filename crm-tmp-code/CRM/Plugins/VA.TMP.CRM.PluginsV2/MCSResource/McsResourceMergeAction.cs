    ...
    public class McsResourceMergeAction : IPlugin
    {
        ...
        private void MergeRecords(IPluginExecutionContext PluginExecutionContext, IOrganizationService OrganizationService, ITracingService tracingService)
        {
            try
            {
                ...
                UpdateServiceAppointmentResources(looserGUID, winnerGUID, entityName, OrganizationService, tracingService);
                tracingService.Trace("After Appointment Records update");
                UpdateAppointmentReserveResources(looserGUID, winnerGUID, entityName, OrganizationService, tracingService);
                tracingService.Trace("After ReserveResource Records update");
                ...
                //Updating merged field with looser GUID
                Entity WinnerUpdate = new Entity(entityName);
                WinnerUpdate.Id = winnerGUID;
                WinnerUpdate["tmp_mergedresource"] = new EntityReference(entityName, looserGUID);
                OrganizationService.Update(WinnerUpdate);
        ...
        private void UpdateAppointmentReserveResources(Guid losingResourceId, Guid winningResourceId, string entityName, IOrganizationService organizationService, ITracingService tracingService)
        {
            try
            {
                #region main logic
                ...
                if (resources != null && !resources.Entities.Count.Equals(0))
                {
                    ...
                    resources.Entities.ToList().ForEach((resource) =>
                    {
                        resource["regardingobjectid"] = new EntityReference(entityName, winningResourceId);
                        organizationService.Update(resource);
                    });
                }
                #endregion
        ...
        private void UpdateServiceAppointmentResources(Guid losingResourceId, Guid winningResourceId, string entityName, IOrganizationService organizationService, ITracingService tracingService)
        {
            try
            {
                #region main logic
