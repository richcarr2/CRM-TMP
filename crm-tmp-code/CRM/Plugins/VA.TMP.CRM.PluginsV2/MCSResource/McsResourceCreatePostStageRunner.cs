    ...
    public class McsResourceCreatePostStageRunner : PluginRunner
    {
        ...
        public override void Execute()
        {
            if (PluginExecutionContext.InputParameters.Contains("Target") && PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                var bookableResourceId = CreateBookableResourceEquipment(equipmentId);
        ...
        private Guid CreateBookableResourceEquipment(Guid equipmentId)
        {
            Equipment thisSource = OrganizationService.Retrieve("equipment", equipmentId, new ColumnSet(true)).ToEntity<Equipment>();

            BookableResource newRecord = new BookableResource();
            newRecord.Id = equipmentId;
            newRecord.CalendarId = thisSource.CalendarId;
            newRecord.UserId = new EntityReference("equipment", thisSource.Id);
            newRecord.EntityState = null;
            newRecord.UTCConversionTimeZoneCode = thisSource.UTCConversionTimeZoneCode;
            //newRecord.msdyn_DeriveCapacity = false;
            //newRecord.msdyn_DisplayOnScheduleAssistant = true;
            //newRecord.msdyn_DisplayOnScheduleBoard = true;
            //newRecord.msdyn_EndLocation = new OptionSetValue(690970002);
            //newRecord.msdyn_StartLocation = new OptionSetValue(690970002);
            return OrganizationService.Create(newRecord);   
        }
        ...
        internal Guid CreateEquipment(Guid ResourceId)
        {
            ...
            try
            {
                using (var srv = new Xrm(OrganizationService))
                {
                    ...
                    Logger.WriteGranularTimingMessage("Initiating Equipment Creation. Name: " + ThisResource.mcs_name);
