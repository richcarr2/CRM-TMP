...
function openNewRecurringAppointmentMaster(subject, serviceActivityID, serviceActivityName) {
    ...
    if ((typeof Xrm !== "undefined") && (typeof Xrm.Utility !== "undefined")) {
        ...
        var extraqs = ["formid=4a8cda55-024e-419c-bbe1-9540e0b8e297", "subject=" + subject, "cvt_serviceactivityid=" + serviceActivityID, "cvt_serviceactivityidname=" + serviceActivityName]
...
EditServiceActivitySeries = function (primaryControl) {
    ...
    if (relatedMaster != null) Xrm.Navigation.openForm("recurringappointmentmaster", relatedMaster[0].id)
}
...
ReOpenServiceActivity = function (primaryControl) {
    ...
    //SD: web-use-strict-equality-operators
    if (isDataAdmin || formContext.getAttribute("ownerid").getValue()[0].id === userSettings.userId) Mscrm.CommandBarActions.activate(formContext.data.entity.getId(), formContext.data.entity.getEntityName());
    else MCS.cvt_Common.openDialogOnCurrentRecord("00520409-98FB-4A1E-B67C-D3D6783ACB84");
}
...
CancelServiceActivity = function (primaryControl) {
    ...
    if (!runVista) {
        ...
        else dialogId = "B8A805D8-01B1-4922-80CA-D4E46F2EC836";
    }
...
CompleteServiceActivity = function (primaryControl) {
    ...
    MCS.Scripts.Process.ExecuteAction(requestName, requestParams).done(function (response) {
...
GetContactDetailsDeferred = function () {
    ...
    function success(result) {
        ...
        //SD: web-use-strict-equality-operators
        if (patRecord.EMailAddress1 !== null && patRecord.EMailAddress1 !== "") allowSave = true;
        //SD: web-use-strict-equality-operators
        if (doNotEMail && (!usingVMR || patRecord.cvt_TabletType === 917290002)) allowSave = true;
...
SaveRecord = function (saveOption, primaryControl) {
    ...
    if (allowSave) formContext.data.entity.save(saveOption);
    else EnterEmail(patients[currentVeteranIndex].id, usingVMR);
};
