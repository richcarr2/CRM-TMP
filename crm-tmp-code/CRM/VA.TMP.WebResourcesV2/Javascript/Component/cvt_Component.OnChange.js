...
MCS.mcs_Component.DisplayFields.init = function (executionContext, webResourceName) {
    ...
    xhr.onreadystatechange = function () {
        MCS.mcs_Component.DisplayFields.completeInitialization(executionContext, xhr);
    };
    xhr.send();
};
...
MCS.mcs_Component.fnMatch = function (executionContext, Node, id) {
    ...
    for (var Types in Node.type) {
        ...
        if (id[0].id != null) varMatch = id[0].name;
        ...
        if (Type.id === varMatch) {
            ...
            else formContext.ui.tabs.get('component_details_tab').sections.get('section_tms').setLabel("");
        }
    }
};
...
MCS.mcs_Component.DefaultManufacturerModel = function (executionContext) {
    ...
    var compType = formContext.getAttribute("cvt_componenttype");
    if (compType.getValue() == null || !compType.getIsDirty()) return;
    ...
    function success(results) {
        ...
        if (results && results.entities.length == 1) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                //SD: web-use-strict-equality-operators
                if (results[i].cvt_Manufacturer.Id !== results.entities[0].cvt_Manufacturer.Id) multipleManufacturers = false;
            }
...
MCS.mcs_Component.PopulateModelDescription = function (executionContext) {
    ...
    if (model.getValue() == null || !model.getIsDirty()) return;
    var id = model.getValue()[0].id;

    Xrm.WebApi.retrieveRecord("cvt_model", id, "?$select=cvt_Description").then(
    function success(result) {
        if (result != null && result.cvt_Description != null) formContext.getAttribute("cvt_modeldescription").setValue(result.cvt_Description);
    },
    function (error) {
        return;
    });
...
MCS.mcs_Component.PopulateDescription = function (executionContext) {
    ...
    if (type.getValue() == null || !type.getIsDirty()) return;
...
MCS.mcs_Component.AddLookupView = function (executionContext) {
    ...
    //return;
    if (formContext.getAttribute("cvt_componenttype").getValue() == null) return;
    ...
    var viewID = type.id;
    var TSAlayoutXML = '<grid name="resultset" object="10034" jump="mcs_name" select="1" icon="0" preview="0">' + '<row name="result" id="cvt_manufacturerid"><cell name="cvt_name" width="300"/>' + '</row></grid>';
    var fetch = '<fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="true"><entity name="cvt_manufacturer">' + '<attribute name="cvt_manufacturerid" /><attribute name="cvt_name" /><order attribute="cvt_name" descending="false" />' + '<link-entity name="cvt_model" from="cvt_manufacturer" to="cvt_manufacturerid" alias="aa">' + '<filter type="and">' + '<condition attribute="cvt_componenttype" operator="eq" uiname="' + MCS.cvt_Common.formatXML(type.name) + '" uitype="cvt_componenttype" value="' + type.id + '" /></filter>' + '</link-entity>' + '</entity></fetch>';
...
MCS.mcs_Component.AddModelLookupView = function (executionContext) {
    ...
    var componentTypeFilter = "<filter type='and'><condition attribute='cvt_componenttype' operator='eq' uiname='" + MCS.cvt_Common.formatXML(type.name) + "' uitype='cvt_componenttype' value='" + type.id + "' /></filter>";
...
MCS.mcs_Component.StatusSetDisplay = function (executionContext) {
    ...
    if (options != null) {
        //SD: web-use-strict-equality-operators
        if (formContext.getAttribute("cvt_status").getValue() !== 917290002) options.removeOption(917290002);
        //SD: web-use-strict-equality-operators
        if (formContext.getAttribute("cvt_status").getValue() !== 917290003) options.removeOption(917290003);
    }
};