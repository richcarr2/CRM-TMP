...
MCS.PPEFeedback.OnLoad = function (executionContext) {
    ...
    if (formContext.getAttribute("cvt_responseescalated").getValue() !== null) {
        ...
        if (formContext.getAttribute("cvt_anythingtoreport").getValue() !== null) formContext.ui.setFormNotification('PPE record had been escalated', 'WARNING');
        else formContext.ui.setFormNotification('PPE record is overdue and has been escalated', 'WARNING');
    }
};
...
MCS.PPEFeedback.GetRelatedTeamMembers = function (executionContext, teamField, subgrid) {
    ...
    if (formContext.ui.getFormType() !== MCS.cvt_Common.FORM_TYPE_CREATE) {
        ...
        if (Grid === null) { //make sure the grids have loaded
            setTimeout(function () {
                MCS.PPEFeedback.GetRelatedTeamMembers(teamField, subgrid);
            },
...
MCS.PPEFeedback.GetFetchXML = function (ThisTeamName, ThisTeamId) {
    var FetchXML = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>" + "<entity name='systemuser'>" + "<attribute name='fullname' />" + "<attribute name='cvt_type' />" + "<attribute name='domainname' />" + "<attribute name='internalemailaddress' />" + "<attribute name='jobtitle' />" + "<attribute name='cvt_site' />" + "<attribute name='cvt_facility' />" + "<attribute name='mcs_visn' />" + "<attribute name='systemuserid' />" +

    "<order attribute='fullname' descending='false' />" + "<link-entity name='teammembership' from='systemuserid' to='systemuserid' visible='false' intersect='true'>" + "<link-entity name='team' from='teamid' to='teamid' alias='ab'>" +

    "<filter type='and'>" + "<condition attribute='teamid' operator='eq' uiname='" + MCS.cvt_Common.formatXML(ThisTeamName) + "' uitype='team' value='" + ThisTeamId + "' />" + "</filter>" + "</link-entity>" + "</link-entity>" + "</entity>" + "</fetch>";
