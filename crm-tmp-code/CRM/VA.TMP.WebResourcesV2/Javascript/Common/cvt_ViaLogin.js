...
{
    MCS.VIALogin.ErrorMessage = null;
    ...
    MCS.VIALogin.UserStationNumbers = [];
}
...
MCS.VIALogin.LoginOnCancelAppointment = function () {
    ...
    var loginOnCancelAppointment = new $.Deferred();
    var loginDeferred = MCS.VIALogin.Login();

    loginDeferred.done(function (returnData) {
        debugger;
        MCS.VIALogin.IsCancelServiceActivityInProgress = false;
        loginOnCancelAppointment.resolve(returnData);
        //return;
    });
    //$.when(loginDeferred).done(function (returnData) {
    //    MCS.VIALogin.IsCancelServiceActivityInProgress = false;
    ...
    return loginOnCancelAppointment.promise();
};
...
MCS.VIALogin.Login = function () {
    ...
    if (checkFacilityTypeDeferred == null) {
        return;
    }
        ...
        if (!returnData.data.isBothSiteFacCerner) {
            setTimeout(function () {
                ...
                $.when(samlTokenDeferred).done(function (validSamlToken) {
                    ...
                    if (validSamlToken) {
                        console.log('SAML is valid.  Checking User Access.');
                        MCS.VIALogin.GetUsersStationNumbers(MCS.VIALogin.SamlString);

                        //var isPrepareFacilitiesDeferred = MCS.VIALogin.PrepareFacilities();
                        var isPrepareFacilitiesDeferred = MCS.VIALogin.CheckUserFacilityAccess();

                        $.when(isPrepareFacilitiesDeferred).done(function (returnData) {
                            console.log("returnData in isPrepareFacilitiesDeferred");
                            ...
                            $.when(isValidUserDuzDeferred).done(function (returnData) {
                                ...
                                if (typeof returnData.data !== 'undefined') {
                                    //if either the returned pro or pat is true, then the UserDuz is true
                                    //otherwise, keep false
                                    if (accessData.data.access.pro || accessData.data.access.pat) {
                                        console.log("accessData.data.access.pro " + accessData.data.access.pro);
                                        console.log("accessData.data.access.pat " + accessData.data.access.pat);
                                        isValidUserDuz = true;
                                    }
                                }
        ...
        $.when(vistaintegrationDeferred).done(function (returnData) {
            ...
            if (returnData.integrationEnabled === false) {
                ...
                $.when(retrieveTokenDeferred).done(function (data) {
                    ...
                    {
                        MCS.VIALogin.ErrorMessage = $(errorTag).text();
    ...
    MCS.VIALogin.UpdateUserSAMLToken = function (saml) {
        ...
        Xrm.WebApi.updateRecord("systemuser", MCS.VIALogin.currentUser, userRecord).then(
            function success(result) {
                ...
                MCS.VIALogin.Login().done(function () {
            ...
            if (returnData.success === false) {
                ...
                $.when(getURLDeferred).done(function (returnData) {
                    ...
                    if (directIamUrl != null && directIamUrl != "")
                        MCS.VIALogin.CallIamSts(directIamUrl).done(function () {
                            deferred.resolve();
                        });
                    else {
...
// ************************************************STS Retrieval Functions****************************************************
{
    ...
    MCS.VIALogin.CallIamSts = function (iamUrl) {
        ...
        var poststring = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ns=\"http://docs.oasis-open.org/ws-sx/ws-trust/200512\">" + "<soapenv:Header/>" + "<soapenv:Body>" + "<ns:RequestSecurityToken>" + "<ns:Base>" + "<wss:TLS xmlns:wss=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\"/>" + "</ns:Base>" + "<wsp:AppliesTo xmlns:wsp=\"http://schemas.xmlsoap.org/ws/2004/09/policy\">" + "<wsa:EndpointReference xmlns:wsa=\"http://schemas.xmlsoap.org/ws/2004/08/addressing\">" + "<wsa:Address>" + url + "/tmp</wsa:Address>" + "</wsa:EndpointReference>" + "</wsp:AppliesTo>" + "<ns:Issuer>" + "<wsa:Address xmlns:wsa=\"http://schemas.xmlsoap.org/ws/2004/08/addressing\">https://ssoi.sts.va.gov/Issuer/smtoken/SAML2</wsa:Address>" + "</ns:Issuer>" + "<ns:RequestType>http://schemas.xmlsoap.org/ws/2005/02/trust/Validate</ns:RequestType>" + "</ns:RequestSecurityToken>" + "</soapenv:Body>" + "</soapenv:Envelope>";
        ...
        $.get(url).done(function () {
            $.ajax({
                ...
                success: function (result) {
                    console.log("SAML");
                    console.log(result);
                    MCS.VIALogin.UpdateUserSAMLToken(result).done(function () {
                        deferred.resolve();
                    });
                    MCS.VIALogin.GetUsersStationNumbers(result);
                    //MCS.VIALogin.Login();
                },
                error: function (jqXHR, tranStatus, errorThrown) {
                    alert("Unable to Get SAML token to log into Vista\n" + 'Status: ' + jqXHR.status + ' ' + jqXHR.statusText + '. ' + 'Response: ' + jqXHR.responseText);
                }
            });
        }).fail(function (err) {
            alert("Failure to Get SAML token to log into Vista\n" + 'Status: ' + err.status + ' ' + err.statusText + '. ' + 'Response: ' + err.responseText);
        });
...
// ************************************************DUZ Checking Functions****************************************************
{
    ...
    MCS.VIALogin.IsValidUserDuz = function () {
                ...
                $.when.all(deferredArray).then(function (returnData) {
                    ...
                    for (n = 0; n < returnData.length; n++) {
                        if (returnData[n].data.Side == "pro") {
                            ...
                            proDuz = MCS.VIALogin.UserStationNumbers.indexOf(MCS.VIALogin.proFacilityNumber) >= 0
                        }
                        if (returnData[n].data.Side == "pat") {
                            MCS.VIALogin.patFacilityNumber = returnData[n].data.facilityNumber;
                            patDuz = MCS.VIALogin.UserStationNumbers.indexOf(MCS.VIALogin.patFacilityNumber) >= 0
                        }
                    }
                    ...
                    if (proDuz && !MCS.VIALogin.isProviderSiteFacCerner) {
                        MCS.VIALogin.ToggleIcon("ProDuz", "success", MCS.VIALogin.proFacilityNumber + ": VistA Authentication Completed");
                        ...
                        if (MCS.VIALogin.proFacilityNumber != null) {
                            ...
                            if (!MCS.VIALogin.isProviderSiteFacCerner) {
                                ...
                                $("#ProAV").show();
                                MCS.VIALogin.ToggleIcon("ProDuz", "failure", MCS.VIALogin.proFacilityNumber + ": VistA login failed. Designated User Account (DUZ) was not found in your SAML token. Please check your access with your local ADPAC.");
                            }
                        }
                        else {
                            if (!MCS.VIALogin.isProviderSiteFacCerner) {
                    ...
                    if (patDuz && !MCS.VIALogin.isPatientSiteFacCerner) {
                        MCS.VIALogin.ToggleIcon("PatDuz", "success", MCS.VIALogin.patFacilityNumber + ": VistA Authentication Completed");
                    }
                    else {
                        if (MCS.VIALogin.patFacilityNumber != null && !MCS.VIALogin.isPatientSiteFacCerner) {
                            ...
                            MCS.VIALogin.ToggleIcon("PatDuz", "failure", MCS.VIALogin.patFacilityNumber + ": VistA login failed. Designated User Account (DUZ) was not found in your SAML token. Please check your access with your local ADPAC.");
                        }
        ...
        //MCS.VIALogin.ValidateUserStationNumbers();
        return deferred.promise();
    };
    ...
    MCS.VIALogin.DuzSide = function () {
        ...
        if (typeCode == 4214) {
            runPatSide = Xrm.Page.getAttribute("cvt_type").getValue() == false || Xrm.Page.getAttribute('cvt_patientsiteresourcesrequired').getValue();
...
//Access and Verify input were always undefined, so set to ""
{
    ...
    MCS.VIALogin.LoginWithAction = function (stationNumber, side, access, verify) {
                    ...
                    switch (side) {
                        case "pat":
                            if (!MCS.VIALogin.isPatientSiteFacCerner) {
                                MCS.VIALogin.ToggleIcon("PatDuz", "failure", stationNumber + ": Failed VistA login with SAML. Designated User Account (DUZ) was not found in your SAML token. Please check your access with your local ADPAC.");
                            }
                            break;
                        case "pro":
                            if (!MCS.VIALogin.isProviderSiteFacCerner) {
                                MCS.VIALogin.ToggleIcon("ProDuz", "failure", stationNumber + ": Failed VistA login with SAML. Designated User Account (DUZ) was not found in your SAML token. Please check your access with your local ADPAC.");
                            }
                            break;
                        case "both":
        ...
        Xrm.WebApi.online.execute(LoginWithActionRequest).then(function (response) {
            if (response.ok) {
                response.json().then(
                    function (responseBody) {
                        ...
                        if (success) {
                            ...
                            $.when(setUserDuzDeferred).done(function (returnData) {
                                // should have been set, verify returnData.success
                                if ((Xrm.Page.getAttribute("cvt_patuserduz") == null & parent.Xrm.Page.getAttribute("cvt_patuserduz") != null)) Xrm = parent.Xrm;
                                ...
                                if (stationNumber == MCS.VIALogin.proFacilityNumber) {
                                    ...
                                    Xrm.Page.getAttribute("cvt_providerstationcode").setValue(MCS.VIALogin.proFacilityNumber);
                                    if (!MCS.VIALogin.isProviderSiteFacCerner) MCS.VIALogin.ToggleIcon("ProDuz", "success", loginType + " successful for " + stationNumber + ".");
                                }
                                if (stationNumber == MCS.VIALogin.patFacilityNumber) {
                                    Xrm.Page.getAttribute("cvt_patuserduz").setValue(userDuz);
                                    if (!MCS.VIALogin.isPatientSiteFacCerner) MCS.VIALogin.ToggleIcon("PatDuz", "success", loginType + " successful for " + stationNumber + ".");
                                }
                                ...
                                if (!MCS.VIALogin.isPatientSiteFacCerner || !MCS.VIALogin.isProviderSiteFacCerner) {
                                    ...
                                    if (stationNumberToVerify !== null) {
                                        ...
                                        $.when(isValidUserDuzDeferred).done(function (isValidUserDuz) {
                                            if (isValidUserDuz) MCS.VIALogin.TriggerGetConsults();
                                        });
                                    }
                                }
                            ...
                            if (MCS.VIALogin.patFacilityNumber == MCS.VIALogin.proFacilityNumber && !MCS.VIALogin.isProviderSiteFacCerner) {
                                MCS.VIALogin.ToggleIcon("ProDuz", "failure", loginType + " failed for " + stationNumber + ". Designated User Account (DUZ) was not found in your SAML token. Please check your access with your local ADPAC.");
                            }
                                ...
                                if (stationNumber == MCS.VIALogin.patFacilityNumber && !MCS.VIALogin.isPatientSiteFacCerner) {
                                    MCS.VIALogin.ToggleIcon("PatDuz", "failure", loginType + " failed for " + stationNumber + ". Designated User Account (DUZ) was not found in your SAML token. Please check your access with your local ADPAC.");
                                }
                            }
    ...
    MCS.VIALogin.TriggerGetConsults = function () {
        ...
        if (Object.keys(Xrm.Utility.getGlobalContext().getQueryStringParameters()).length === 0) Xrm = parent.Xrm;

        //var typecode = "4214";
        var typeCode = Xrm.Utility.getGlobalContext().getQueryStringParameters().etc;
        var entityTypeName = Xrm.Utility.getGlobalContext().getQueryStringParameters().entityTypeName;
        typeCode = entityTypeName === "serviceappointment" ? 4214 : 4201;
        if (typeCode == 4214) Xrm.Page.getAttribute("cvt_rungetconsults").fireOnChange();
    };
...
// ************************VistA Switches and Integration checks *************************************
{
    MCS.VIALogin.RunVistaIntegration = function () {
        ...
        $.when(switchesDeferred).done(function (data) {
            ...
            if (typeof Xrm != "undefined") appointmentSwitchDeferred = MCS.VIALogin.AppointmentTypeSwitchCheck(data.hmConfig, data.ifcConfig, data.singleNonHmConfig, Xrm);
            else appointmentSwitchDeferred = MCS.VIALogin.AppointmentTypeSwitchCheck(data.hmConfig, data.ifcConfig, data.singleNonHmConfig, parent.Xrm);
        ...
        Xrm.WebApi.retrieveRecord('cvt_resourcepackage', tsaId, "?$select=_cvt_specialty_value, _cvt_specialtysubtype_value").then(
            function success(result) {
                if (result != null) {
                    ...
                    if (typeof result._cvt_specialty_value != 'undefined' && result._cvt_specialty_value !== null) returnData.data.specialty = result._cvt_specialty_value;

                    if (typeof result._cvt_specialtysubtype_value != 'undefined' && result._cvt_specialtysubtype_value !== null) returnData.data.subSpecialty = result._cvt_specialtysubtype_value;
    ...
    MCS.VIALogin.AppointmentTypeSwitchCheck = function (hmConfig, ifcConfig, sftConfig, Xrm) {
        ...
        //Appt so use this record
            ...
            {
                if ((Xrm.Page.getAttribute("cvt_relatedschedulingpackage")) && (Xrm.Page.getAttribute("cvt_relatedschedulingpackage").getValue() != null)) tsaId = Xrm.Page.getAttribute("cvt_relatedschedulingpackage").getValue()[0].id;
...
MCS.VIALogin.checkFacilityType = function () {
    ...
    if (typeCode == 4214) {
        ...
        if (schedulingPackage.getValue() == null) {
            return;
        }
        ...
        $.when(_completeFacilityTypeCheck).done(function (returnCFTC) {
            ...
            if (returnCFTC.data.isProviderSiteFacCerner != undefined && returnCFTC.data.isProviderSiteFacCerner != "") {
                returnData.data.isProviderSiteFacCerner = returnCFTC.data.isProviderSiteFacCerner;
            }
            if (returnCFTC.data.isPatientSiteFacCerner != undefined && returnCFTC.data.isPatientSiteFacCerner != "") {
        ...
        var queryStringParams = Xrm.Utility.getGlobalContext().getQueryStringParameters();

        if (queryStringParams.id != null) {
            var rrid = queryStringParams.id.replace("{", "").replace("}", "");
...
MCS.VIALogin.ToggleIcon = function (area, toggle, message) {
    ...
    var icons = [
        $("#msg" + area + "_working"), $("#msg" + area + "_success"), $("#msg" + area + "_failure")];
...
// ************************    Validate Facilities User Can Access    *************************************
// ************************ These are replacing the userduz functions *************************************
MCS.VIALogin.GetUsersStationNumbers = function (saml) {
    console.log("Begin GetUsersStationNumbers");
    console.log(saml);

    //jquery to check SAML token for user's VistA facilities
    var userStationNumberElements = $(saml).find("saml\\:Attribute[Name^='urn:va:vrm:iam:vistaid']").find("saml\\:AttributeValue");
    if (userStationNumberElements != undefined && userStationNumberElements.length > 0) {
        //verify if the User Station Numbers haven't changed (i.e. added or removed any Ids)
        if (userStationNumberElements.length != MCS.VIALogin.UserStationNumbers.length) MCS.VIALogin.UserStationNumbers = [];

        //add the VistId to the array if it doesn't exist in the array
        userStationNumberElements.each(function (idx, vistaIdElement) {
            var vistaId = $(vistaIdElement).text().substring(0, 3);
            console.log(vistaId);
            if (MCS.VIALogin.UserStationNumbers.indexOf(vistaId) < 0) MCS.VIALogin.UserStationNumbers.push(vistaId); //add facilities to global variable
        });
    }
    console.log(MCS.VIALogin.UserStationNumbers);

    console.log("End GetUsersStationNumbers");
};

MCS.VIALogin.CheckUserFacilityAccess = function () {
    console.log("Begin CheckUserFacilityAccess");
    ...
    //set facility station number variables
    var proFacility = MCS.VIALogin.proFacilityNumber;
    var patFacility = MCS.VIALogin.patFacilityNumber;
    //set array for user's VistA facilities' station numbers
    //do final check for User Station Numbers to make sure we have them
    if (MCS.VIALogin.UserStationNumbers == undefined || MCS.VIALogin.UserStationNumbers == null || MCS.VIALogin.UserStationNumbers.length === 0) {
        if (MCS.VIALogin.SamlString == "") {
            MCS.VIALogin.Saml();
        }
        else MCS.VIALogin.GetUsersStationNumbers(MCS.VIALogin.SamlString);
    }
    var userNumbers = MCS.VIALogin.UserStationNumbers;
    //initalize array to hold whether or not user's facilities match
    var facilityAccess = {
        pro: false,
        pat: false
    }

    //check if user's facilities match either pro or pat facility
    console.log("userNumbers:");
    console.log(userNumbers);

    facilityAccess.pro = userNumbers.indexOf(proFacility) >= 0;
    facilityAccess.pat = userNumbers.indexOf(patFacility) >= 0;

    returnData.data.access = facilityAccess;
    deferred.resolve(returnData);

    console.log("End CheckUserFacilityAccess");
