...
MCS.Scripts.Process = function () {
    ...
    var soapParams = function (paramArray, genericNSPrefix, schemaNSPrefix) {
        ...
        if (paramArray) {
            ...
            for (var i = 0; i < paramArray.length; i++) {
                ...
                params += "<a:KeyValuePairOfstringanyType>" + "<" + genericNSPrefix + ":key>" + param.Key + "</" + genericNSPrefix + ":key>" + "<" + genericNSPrefix + ":value i:type='" + type + "'";
                if (includeNS) params += " xmlns:" + schemaNSPrefix + "='" + typeNS + "'";
                params += ">" + value + "</" + genericNSPrefix + ":value>" + "</a:KeyValuePairOfstringanyType>";
            }
        }

        return "<a:Parameters xmlns:" + genericNSPrefix + "='http://schemas.datacontract.org/2004/07/System.Collections.Generic'>" + params + "</a:Parameters>";
    };

    var soapExecute = function (requestXml) {
        return "<Execute xmlns='http://schemas.microsoft.com/xrm/2011/Contracts/Services' xmlns:i='http://www.w3.org/2001/XMLSchema-instance'>" + requestXml + "</Execute>";
    };

    var soapEnvelope = function (message) {
        return "<s:Envelope xmlns:s='http://schemas.xmlsoap.org/soap/envelope/'>" + "<s:Body>" + message + "</s:Body>" + "</s:Envelope>";
    };

    var soapActionRequest = function (action, inputParams) {
        return "<request xmlns:a='http://schemas.microsoft.com/xrm/2011/Contracts'>" + soapParams(inputParams, 'b', 'c') + "<a:RequestId i:nil='true' />" + "<a:RequestName>" + action + "</a:RequestName>" + "</request>";
    };

    var soapExecuteWorkflowRequest = function (workflowId, recordId) {
        return "<request i:type='b:ExecuteWorkflowRequest' xmlns:a='http://schemas.microsoft.com/xrm/2011/Contracts' xmlns:b='http://schemas.microsoft.com/crm/2011/Contracts'>" + soapParams([{
            Key: "EntityId",
            Type: DataTypes.Guid,
            Value: recordId
        },
        {
            ...
            Type: DataTypes.Guid,
            Value: workflowId
        }], 'c', 'd') + "<a:RequestId i:nil='true' />" + "<a:RequestName>ExecuteWorkflow</a:RequestName>" + "</request>";
    };
    ...
    var callAction = function (action, inputParams) {
        ...
        execCrmSoapRequest(soapEnvelope(soapExecute(soapActionRequest(action, inputParams)))).done(function (a, b, xhr) {
