...
MCS.GroupResource.HandleOnChangeLookup = function (executionContext) {
    ...
    if (type != null && site != null) {
        ...
        */
        ...
        var ResourceColumns = ['mcs_resourceid', 'mcs_name', 'createdon'];
        var ResourceConditions = ['<condition attribute="statecode" operator="eq" value="0"/>', '<condition attribute="mcs_relatedsiteid" operator="eq" uiname="' + MCS.cvt_Common.formatXML(site[0].name) + '" uitype="mcs_site" value="' + site[0].id + '"/>'];
        var ResourceOrder = ['mcs_name', false];
        ...
        var UserColumns = ['fullname', 'title', 'address1_telephone1', 'businessunitid', 'cvt_type', 'systemuserid'];
        var UserOrder = ['fullname', false];
        var UserConditions = ['<condition attribute="isdisabled" operator="eq" value="0"/>', '<condition attribute="accessmode" operator="ne" value="3"/>', '<condition attribute="cvt_type" operator="eq" value="917290001"/>', '<condition attribute="cvt_site" operator="eq" uiname="' + MCS.cvt_Common.formatXML(site[0].name) + '" uitype="mcs_site" value="' + site[0].id + '"/>'];
...
MCS.GroupResource.CreateName = function (executionContext) {
    ...
    if (formContext.getControl("mcs_relatedresourceid").getVisible() && formContext.getAttribute("mcs_relatedresourceid").getValue() != null) builtName = formContext.getAttribute("mcs_relatedresourceid").getValue()[0].name;

    if (formContext.getControl("mcs_relateduserid").getVisible() && formContext.getAttribute("mcs_relateduserid").getValue() != null) builtName = formContext.getAttribute("mcs_relateduserid").getValue()[0].name;
    if (builtName === "") {
        var alertStrings = {
            confirmButtonLabel: "OK",
            text: "Please pick the related resource or user before saving.",
            title: "user before saving."
        };
