...
MCS.mcs_Resource_Buttons.openResourceCalendar = function (formContext) {
    ...
    if (relatedSystemResource != null) {
        ...
        if (relatedSystemResourceValue[0] != null) relatedSystemResourceId = relatedSystemResourceValue[0].id;
    }
...
MCS.mcs_Resource_Buttons.ReplaceResource = function (primaryControl) {
    ...
    if (confirm("Please confirm that you would like to replace this TMP Resource on all MTSA's, TSA's, and Resource Groups. Please Select Ok to continue or Cancel to abort.")) MCS.cvt_Common.openDialogOnCurrentRecord(primaryControl, "26CDD7B6-7A94-47E2-BC3A-1169722E81F0");
    else formContext.getAttribute("cvt_replaceresourceconnections").setValue(false);
};
