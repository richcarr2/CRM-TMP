...
MCS.mcs_Resource.RemoveProTeleOptions = function (executionContext) {
    ...
    if (formContext.getAttribute("mcs_type").getValue() != null) formContext.getControl("mcs_type").setDisabled(true);

    //Submit if OnCreate
    if (formContext.ui.getFormType() === MCS.cvt_Common.FORM_TYPE_CREATE) formContext.getAttribute("mcs_type").setSubmitMode("always");
};
...
MCS.mcs_Resource.GetRelatedPSs = function (executionContext) {
    ...
    if (formContext.ui.getFormType() !== MCS.cvt_Common.FORM_TYPE_CREATE) {
        ...
        if (PSGrid == null) { //make sure the grids have loaded
            setTimeout(function () {
                MCS.mcs_Resource.GetRelatedPSs();
            },
            500); //if the grid hasn’t loaded run this again when it has
            return;
        }
...
MCS.mcs_Resource.GetRelatedServiceActivities = function (executionContext) {
    ...
    if (formContext.ui.getFormType() !== MCS.cvt_Common.FORM_TYPE_CREATE) {
        ...
        if (SAGrid == null) { //make sure the grids have loaded
            setTimeout(function () {
                MCS.mcs_Resource.GetRelatedServiceActivities();
            },
            500); //if the grid hasn’t loaded run this again when it has
            return;
        }
...
MCS.mcs_Resource.GetScheduledAppointments = function (executionContext) {
    ...
    if (formContext.ui.getFormType() !== MCS.cvt_Common.FORM_TYPE_CREATE) {
        ...
        if (AptGrid == null) { //make sure the grids have loaded
            setTimeout(function () {
                MCS.mcs_Resource.GetScheduledAppointments();
            },
            500); //if the grid hasn’t loaded run this again when it has
            return;
        }
...
MCS.mcs_Resource.GetFetchXML = function (entityName, ThisResourceName, ThisResourceId) {
    ...
    case "cvt_patientresourcegroup":
        ...
        var FetchXML = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>" + "<entity name='mcs_services'>" + "<attribute name='mcs_name' />" + "<attribute name='createdon' />" + "<attribute name='cvt_relatedprovidersiteid' />" + "<attribute name='cvt_relatedpatientsiteid' />" + "<attribute name='cvt_relatedmasterid' />" + "<attribute name='cvt_servicetype' />" + "<attribute name='cvt_servicesubtype' />" + "<attribute name='statuscode' />" + "<attribute name='ownerid' />" + "<attribute name='modifiedon' />" + "<attribute name='cvt_servicescope' />" + "<attribute name='cvt_groupappointment' />" + "<attribute name='cvt_provsitevistaclinics' />" + "<attribute name='cvt_patsitevistaclinics' />" + "<attribute name='mcs_servicesid' />" + "<order attribute='mcs_name' descending='false' />" + "<filter type='and'>" + "<condition attribute='statecode' operator='eq' value='0' />" + "</filter>" + "<link-entity name='" + entityName + "' from='cvt_relatedtsaid' to='mcs_servicesid' alias='ai'>" + "<filter type='and'>" + "<condition attribute='cvt_relatedresourceid' operator='eq' uiname='" + MCS.cvt_Common.formatXML(ThisResourceName) + "' uitype='mcs_resource' value='" + ThisResourceId + "' />" + "</filter>" + "</link-entity>" + "</entity>" + "</fetch>";
        break;
    case "serviceappointment":
        var FetchXML = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>" + "<entity name='activitypointer'>" + "<attribute name='activitytypecode' />" + "<attribute name='subject' />" + "<attribute name='statecode' />" + "<attribute name='prioritycode' />" + "<attribute name='modifiedon' />" + "<attribute name='activityid' />" + "<attribute name='instancetypecode' />" + "<order attribute='modifiedon' descending='false' />" + "<filter type='and'>" + "<condition attribute='activitytypecode' operator='eq' value='4214' />" + "<condition attribute='scheduledstart' operator='next-x-days' value='365' />" + "</filter>" + "<link-entity name='activityparty' from='activityid' to='activityid' alias='al'>" + "<filter type='and'>" + "<condition attribute='partyid' operator='eq' uiname='" + MCS.cvt_Common.formatXML(ThisResourceName) + "' uitype='equipment' value='" + ThisResourceId + "' />" + "</filter>" + "</link-entity>" + "</entity>" + "</fetch>";
        break;
    case "appointment":
        FetchXML = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>" + "<entity name='activitypointer'>" + "<attribute name='activitytypecode' />" + "<attribute name='subject' />" + "<attribute name='statecode' />" + "<attribute name='prioritycode' />" + "<attribute name='modifiedon' />" + "<attribute name='activityid' />" + "<attribute name='instancetypecode' />" + "<order attribute='modifiedon' descending='false' />" + "<filter type='and'>" + "<condition attribute='activitytypecode' operator='eq' value='4201' />" + "<condition attribute='scheduledstart' operator='next-x-days' value='365' />" + "</filter>" + "<link-entity name='activityparty' from='activityid' to='activityid' alias='al'>" + "<filter type='and'>" + "<condition attribute='partyid' operator='eq' uiname='" + MCS.cvt_Common.formatXML(ThisResourceName) + "' uitype='equipment' value='" + ThisResourceId + "' />" + "</filter>" + "</link-entity>" + "</entity>" + "</fetch>";
        break;
    case "cvt_schedulingresource":
        //fetch xml code which will retrieve all PS's Related to this Resource.
        FetchXML = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>" + "<entity name='cvt_participatingsite'>" + "<attribute name='cvt_name' />" + "<attribute name='cvt_site' />" + "<attribute name='cvt_resourcepackageid' />" + "<attribute name='cvt_locationtype' />" + "<attribute name='cvt_scheduleable' />" + "<attribute name='cvt_participatingsiteid' />" + "<attribute name='cvt_participatingsiteid' />" + "<order attribute='cvt_name' descending='false' />" + "<order attribute='cvt_resourcepackage' descending='false' />" + "<link-entity name='" + entityName + "' from='cvt_participatingsite' to='cvt_participatingsiteid' alias='ad'>" + "<filter type='and'>" + "<condition attribute='cvt_tmpresource' operator='eq' uiname='" + MCS.cvt_Common.formatXML(ThisResourceName) + "' uitype='mcs_resource' value='" + ThisResourceId + "' />" + "</filter>" + "</link-entity>" + "</entity>" + "</fetch>";
        break;
    }
    return FetchXML;
};
...
MCS.mcs_Resource.CreateName = function (executionContext) {
    ...
    case 251920001:
        ...
        if ((derivedResultField === "") && (mcs_usernameinput !== null)) derivedResultField = mcs_usernameinput;
        break;
    case 251920002:
    ...
    if (formContext.getAttribute("mcs_relatedsiteid").getValue() !== null) derivedResultField += " @ " + formContext.getAttribute("mcs_relatedsiteid").getValue()[0].name;
...
MCS.mcs_Resource.MakeFieldsReadOnly = function (executionContext) {
    ...
    if (formContext.ui.getFormType() !== MCS.cvt_Common.FORM_TYPE_CREATE) {
        ...
        if (formContext.getAttribute("mcs_type").getValue() !== 251920002) formContext.getControl("mcs_relatedsiteid").setDisabled(true);
    }
};
...
MCS.mcs_Resource.CheckIEN = function (ec) {
    ...
    if (rType === 251920000) {
        ...
        if (!_result) {
            ...
            var alertStrings = {
                confirmButtonLabel: "OK",
                text: "The Vista IEN field contains non-numeric characters.  Please correct and resave.",
                title: "Vista Rule"
            };
            Xrm.Navigation.openAlertDialog(alertStrings, alertOptions)
...
MCS.mcs_Resource.ResourceType = function (executionContext) {
    ...
    if (resourceTypeIsTech) {
        ...
        if (formContext.getAttribute("cvt_systemtype").getValue() === 917290005) {
            if (formContext.getAttribute("cvt_locationuse").getValue() != null) formContext.getAttribute("cvt_locationuse").setValue(917290001);
        ...
        else formContext.getControl("cvt_room").setDisabled(false);
    }
