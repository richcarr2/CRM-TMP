function BuildJSTree(formContext, selectedItem) {
    ...
    if (selectedItem.getGrid().controlName == "relatedProviderSites") { //Provider
        ...
        if (providerFacility) {
            ...
            getExistingSites(resourcePackageId, locationTypes.PROVIDER).done(function (results) {
        ...
        if (groupAppointment && patientFacility) {
            if (patientFacility) {
                ...
                getExistingSites(resourcePackageId, locationTypes.PATIENT).done(function (results) {
            ...
            if (this.status === 200) {
                ...
                for (var i = 0; i < results.value.length; i++) {
                    if (!parent) {
                        ...
                        tree = {
                            "id": results.value[j]._mcs_facilityid_value,
                            "parent": "#",
                            "text": results.value[j]["_mcs_facilityid_value@OData.Community.Display.V1.FormattedValue"]
                        }
                        output.push(tree);
                    }
                    if (existingSites.indexOf(results.value[i].mcs_siteid) >= 0) {
                        tree = {
                            "id": results.value[i].mcs_siteid,
                            "parent": results.value[j]._mcs_facilityid_value,
                            "text": results.value[i].mcs_name,
                            "state": {
                                "disabled": true
                            }
                        }
                ...
                Xrm.Navigation.navigateTo(pageInput, navigationOptions).then(
                    function success() {
                        ...
                        if (locationTypeName === "provider") formContext.getControl("relatedProviderSites").refresh();
                        else formContext.getControl("relatedPatientSites").refresh();
                    },
                    function error() {
...
function BuildPatientInterFacilityFullJson(formContext, resourcePackageId, locationTypeName, locationType, popupTitle, existingSites, providerFacility) {
    debugger;
    var req = new XMLHttpRequest();
    req.open("GET", Xrm.Utility.getGlobalContext().getClientUrl() + "/api/data/v9.1/mcs_sites?$select=_mcs_facilityid_value,mcs_name,_mcs_businessunitid_value&$expand=mcs_facilityid($select=mcs_businessunitid,mcs_facilityid)&$filter=statecode eq 0 and _mcs_facilityid_value ne " + providerFacility.getValue()[0].id.replace("{", "").replace("}", ""), false);
    req.setRequestHeader("OData-MaxVersion", "4.0");
    req.setRequestHeader("OData-Version", "4.0");
    req.setRequestHeader("Accept", "application/json");
    req.setRequestHeader("Content-Type", "application/json; charset=utf-8");
