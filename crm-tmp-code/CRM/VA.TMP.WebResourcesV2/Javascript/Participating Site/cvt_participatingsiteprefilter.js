...
/*This will be used to hold the value of either the provider or patient facility globally
so that it can be called in multiple functions.
This is done because the filter code is running multiple times when the Location Type field
is changed. The first time it runs it uses the previouly passed in facilityid. */
var globalFacilityId = 0;
...
MCS.ParticipatingSiteOnLoad.filterSites = function (executionContext) {
    ...
    $.when(SiteFilterDeferred).done(function (returnData) {
        ...
        if (!_grpAppt) {
            console.log("in INDIVIDUAL Appointment Branch.  Next: Checking for Side");
            if (!_inter) //not an interfacility appointment
            {
                if (_psSide === "pro") {
                    console.log("calling PROVIDER-INDIVIDUAL filter!");
                    globalFacilityId = _proFac;
...
MCS.ParticipatingSiteOnLoad.addCustomSiteFilter = function (facilityId, formContext) {
    ...
    //prevent the code from running multiple times
    //otherwise it does not use the correct filter.
    if (facilityId == globalFacilityId) {
        if (typeof facilityId !== 'undefined') {
            ...
            console.log("facilityId is undefined")
