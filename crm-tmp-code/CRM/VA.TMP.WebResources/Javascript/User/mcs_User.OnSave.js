    ...
    if (formContext.ui.getFormType() !== 1) {
        ...
        if (!canEdit) {
            ...
            if (isCurrentUsersRecord) {
                if (formContext.getControl("cvt_tsaproviderpreferences") != null)
                    formContext.getControl("cvt_tsaproviderpreferences").setDisabled(false);
            }
        }
...
MCS.User.SetSubmit = function (executionContext) {
    var formContext = executionContext.getFormContext();
    //SD: web-use-strict-equality-operators
    if (formContext.getAttribute("cvt_deleteuserconnections").getIsDirty() === true)
        formContext.getAttribute("cvt_deleteuserconnections").setSubmitMode("always");

    //SD: web-use-strict-equality-operators
    if (formContext.getAttribute("cvt_updateuserconnections").getIsDirty() === true)
        formContext.getAttribute("cvt_updateuserconnections").setSubmitMode("always");

    //SD: web-use-strict-equality-operators
    if (formContext.getAttribute("cvt_replacementuser").getIsDirty() === true)
        formContext.getAttribute("cvt_replacementuser").setSubmitMode("always");
};
...
MCS.User.DefaultProviderPreferences = function (executionContext) {
    ...
    if (userType === 917290001) {
        ...
        formContext.ui.tabs.get('SUMMARY_TAB').sections.get('ProviderPreferences').setVisible(false);
};
...
MCS.User.VerifyLocations = function (executionContext) {
    ...
    if (formContext.getAttribute('businessunitid').getValue() != null)
        visn = formContext.getAttribute('businessunitid').getValue()[0].id;
    else {
    ...
    if (formContext.getAttribute('cvt_facility').getValue() != null)
        facility = formContext.getAttribute('cvt_facility').getValue()[0].id;
    else {
    ...
    if (formContext.getAttribute('cvt_site').getValue() != null)
        site = formContext.getAttribute('cvt_site').getValue()[0].id;
    else {
...
MCS.User.onLoad = function (executionContext) {
    ...
    //SD: web-use-strict-equality-operators
    if (formContext.getAttribute('cvt_type').getValue() != null &&
        (formContext.getAttribute('cvt_type').getValue() === 917290001 ||
            formContext.getAttribute('cvt_type').getValue() === 917290005 ||
            formContext.getAttribute('cvt_type').getValue() === 917290000)) {
