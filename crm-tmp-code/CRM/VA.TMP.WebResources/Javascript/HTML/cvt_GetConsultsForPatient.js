...
GetConsultsForPatient.GetValues = function (data, row, counter) {
    ...
    if ((status != "Pending") && (status != "Active")) {
        if (filterVal == "P") { row.setAttribute("style", "display:none"); }
    ...
    if (filterDate != null) {
        ...
        if (filterDate == "Past30" && ageInDays == "30") {
            if ((status != "Pending" && status != "Active") && filterVal == "P")
                row.setAttribute("style", "display:none");
            else
                row.setAttribute("style", "");
        }
        ...
        function success(results) {
            if (results && results.entities != null) {
                ...
                if (typeof Xrm != "undefined")
                    appointmentTypeSwitchCheckDeferred = MCS.VIALogin.AppointmentTypeSwitchCheck(hmConfig, ifcConfig, singleNonHmConfig, Xrm);
                else
                    appointmentTypeSwitchCheckDeferred = MCS.VIALogin.AppointmentTypeSwitchCheck(hmConfig, ifcConfig, singleNonHmConfig, parent.Xrm);
...
GetConsultsForPatient.SetRtcValues = function (selectedRow) {
    ...
    if (cvt_loc == "patient") {
        ...
        if (patrtcId == null || patrtcId == "")
            Xrm.Page.getAttribute("cvt_patrtcid").setValue(rtcId);

        var patrtcParentId = formContext.getAttribute("cvt_rtcparentpatient").getValue();
        if (patrtcParentId == null || patrtcParentId == "")
            Xrm.Page.getAttribute("cvt_rtcparentpatient").setValue(rtcParent);
    }

    $("#selectedConsult").append("<br/>Added Return to Clinic with RTC Id: " + rtcId + " to appointment booking on " + cvt_loc + " side.");
    $("#informUserConsultAdded").show();
