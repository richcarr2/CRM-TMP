...
MCS.ParticipatingSite.OnLoad = function (executionContext) {
	...
	if (formContext.ui.getFormType() !== 1) {
		...
		Xrm.WebApi.retrieveRecord("cvt_resourcepackage", formContext.getAttribute("cvt_resourcepackage").getValue()[0].id, "?$select=cvt_usagetype").then(
			...
			function (error) {
				...
				var alertStrings = { confirmButtonLabel: "OK", text: "cvt_resourcepackage retrieve error: " + error.message, title: "cvt_resourcepackage retrive error." };
			...
			function success(result) {
				if (result) {
					...
					if (result.cvt_availabletelehealthmodality) {
						if (result.cvt_availabletelehealthmodality.Value === 917290000)
...
MCS.PS_Buttons.buildshowStandardDialog = function (entityName, viewName, callback) {
	...
	if (MCS.ParticipatingSite.Site !== null) {
		...
		var lookupOptions =
		{
			...
			filters: [{ filterXml: FetchFilters, entityLogicalName: entityName }]
		};
			...
			for (i = 0; i < returnedItems.length; i++) {
				...
				Xrm.WebApi.retrieveRecord(entityName, ResourceId, "?$select=mcs_name,mcs_type").then(
					...
					function (error) {
						...
						var alertOptions = { height: 200, width: 300 };
						var alertStrings = { confirmButtonLabel: "OK", text: "Failed retrieved" + MCS.cvt_Common.RestError(error), title: "failed retrived rule." };
						Xrm.Navigation.openAlertDialog(alertStrings, alertOptions);
					}
				);
			}
