...
namespace VA.TMP.Integration.Plugins.ServiceAppointment
{
    ...
    /// </summary>
    public class ServiceAppointmentVvsUpdatePostStageRunner : AILogicBase
    {
        /// <summary>
        /// Default constructor.
        /// </summary>
        ...
        public override void ExecuteLogic()
        {
            ...
            if (sa.cvt_TelephoneCall.HasValue && sa.cvt_TelephoneCall.Value)
            {
                ...
                Trace("This is a Telephone Call Appointment, hence skipping VVS integration.", LogLevel.Debug);
                Success = true;
                return;
            }
            ...
            using (var context = new Xrm(OrganizationService))
            {
                ...
                {
                    VistaTypeFacility = CernerHelper.CheckIfRelatedCernerFacility(sa, context, pluginLogger);
                ...
                if (VistaTypeFacility)
                {
                    ...
                    if (VistaPluginHelpers.RunVvs(sa, context, pluginLogger, PluginExecutionContext))
                    {
                        ...
                        else
                        {
                            ...
                            Trace("Service Activity not in Proper 'Pending' or 'Interface VIMT Failure' status for writing Vista Results back into TMP, skipping VVS.", LogLevel.Debug);
                        }
                    }
                    else
                    {
                        //Logger.WriteDebugMessage("VVS Integration Bypassed, not updating Service Activity Status");
                        Trace("VVS Integration Bypassed, not updating Service Activity Status.", LogLevel.Debug);
                        Success = true;
                    }
                }
                else
                {
                    ...
                    Trace("The Current Facility IS NOT a Vista Facility. Vista Processing halted; Control NOT Passed to Logic App becuse VVS is not applicable to Cerner.", LogLevel.Debug);
                    Success = true;
                }
            }
        }
        ...
        private void SendVistaMessage(DataModel.ServiceAppointment serviceAppointment)
        {
            ...
            if (Veterans != null)
            {
                ...
                Trace("Using Patients passed in from Proxy Add Plugin.", LogLevel.Debug);
            ...
            else
            {
                ...
                changedPatients = VistaPluginHelpers.GetChangedPatients(serviceAppointment, OrganizationService, pluginLogger, out isBookRequest);
            }
            ...
            if (isBookRequest)
            {
                ...
                if (!isUpdate)
                {
                    ...
                    Trace("Sending Create Booking to Vista.", LogLevel.Debug);
                    ...
                    Trace("Create request {request}.", LogLevel.Debug);
                ...
                else
                {
                    ...
                    Trace("Sending Update Booking to Vista.", LogLevel.Debug);
                    ...
                    Trace("Update request {request}.", LogLevel.Debug);
            ...
            else
            {
                ...
                Trace("No Booked Patients.  Individual patient(s) was canceled through Cancel Dialog and has already been sent to VVS in previous plugin instance.  No action needed here, ending thread.  Canceled Patients: " + patIds, LogLevel.Debug);
            }
        }
        ...
        private void ProcessVistaBookResponse(bool isCreate, DataModel.ServiceAppointment serviceAppointment, string errorMessage, bool exceptionOccured, string vimtRequest, string serializedInstance, string vimtResponse, int EcProcessingTime, int vimtProcessingTime, int lagTime, WriteResults writeResults)
        {
            ...
            if (exceptionOccured)
            {
                ...
                Trace("VVS Booking Failed, not updating Service Activity Status.", LogLevel.Debug);
                return;
            }
