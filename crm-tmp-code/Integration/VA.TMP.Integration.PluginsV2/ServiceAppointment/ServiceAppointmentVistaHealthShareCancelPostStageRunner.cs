...
{
    public class ServiceAppointmentVistaHealthShareCancelPostStageRunner : AILogicBase
    {
        ...
        public override void ExecuteLogic()
        {
            ...
            {
                filter = VistaPluginHelpers.RunVistaIntegration(saRecord, context, Logger, PluginExecutionContext);
                //Logger.WriteDebugMessage($"Run HealthShare Cancel after checking switches is set to {filter}");
                Trace($"Run HealthShare Cancel after checking switches is set to {filter}.", LogLevel.Debug);
            ...
            if (saRecord.cvt_Type != null && (saRecord.mcs_groupappointment == null || !saRecord.mcs_groupappointment.Value || saRecord.cvt_Type.Value))
            {
                if (saRecord.StateCode != null && saRecord.StateCode.Value == ServiceAppointmentState.Canceled)
                {
                    ...
                    else // VIA integrations are turned off
                    {
                        ...
                        Trace("Vista Integration (through VIA/HealthShare) is turned off system wide, skipping Vista Cancel integration.", LogLevel.Debug);
                        Success = true;
                    }
                }
                else
                {
                    ...
                    Trace("Vista Cancel Integration skipped - Appointment not in 'Canceled' state.", LogLevel.Debug);
                    Success = true;
                }
            }
            else
            {
                ...
                Trace("Cancel Request invalid for Clinic Based Group Appointments.", LogLevel.Debug);
                Success = true;
            }
        ...
        private bool RunCancelVistaBooking(DataModel.ServiceAppointment sa, string orgName, SystemUser user, bool VistaTypeFacility)
        {
            ...
            try
            {
                if (VistaTypeFacility)
                {
                    ...
                    Trace($"Sending HealthShare Vista Cancel Appointment Request Message to VIMT: {vimtRequest}.", LogLevel.Debug);
                    ...
                    Trace($"Finished Sending HealthShare Vista Cancel Appointment Request Message to VIMT for Service Appointment with Id: {PrimaryEntity.Id}.", LogLevel.Debug);
                    ...
                    Trace($"SA status for HealthShare Cancel is {sa.StatusCode.Value}.", LogLevel.Debug);
            ...
            Trace($"For HealthShare Cancel anyfailures is set to {anyfailures}.", LogLevel.Debug);
        ...
        private int ProcessCernerResponse(TmpCernerOutboundResponseMessage response, DataModel.ServiceAppointment sa)
        {
            ...
            if (exceptionOccured)
            {
                //Logger.WriteDebugMessage($"The was an error in the response for Cerner Cancel: {errMessage}");
                Trace($"The was an error in the response for Cerner Cancel: {errMessage}.", LogLevel.Debug);
            }
            ...
            Trace($"Status returned after Cerner Cancel process response is {status}.", LogLevel.Debug);
        ...
        private int ProcessVistaResponse(TmpHealthShareMakeCancelOutboundResponseMessage response, DataModel.ServiceAppointment sa)
        {
            ...
            if (exceptionOccured)
            {
                //Logger.WriteDebugMessage($"The was an error in the response for HealthShare Cancel: {errMessage}");
                Trace($"The was an error in the response for HealthShare Cancel: {errMessage}.", LogLevel.Debug);
            }
            ...
            Trace($"Status returned after HealthShare Cancel process response is {status}.", LogLevel.Debug);
