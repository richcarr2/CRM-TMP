...
namespace VA.TMP.Integration.Plugins.Helpers
{
    ...
    public class AzureTokenHelper
    {
        ...
        /// <param name="logger">Logger.</param>
        /// <returns>Azure Access Token.</returns>
        public static string GetAzureToken(string resource, string appId, string secret, string authority, string tenantId, AI.PluginLogger logger = null)
        {
            using (var httpClient = new HttpClient())
            {
                var formContent = new FormUrlEncodedContent(new[]
        ...
        {
            if (logger != null) logger.Trace($"TOKEN: {message}", AI.LogLevel.Debug);
        }
    }
}
