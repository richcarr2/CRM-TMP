...
namespace VA.TMP.Integration.VirtualMeetingRoom.MessageHandler
{
    ...
    /// </summary>
    public class VirtualMeetingRoomCreateHandler
    {
        ...
        public VirtualMeetingRoomCreateHandler(ILog logger, Settings settings)
        {
            _logger = logger;
            _settings = settings;
        }
        ...
        public VirtualMeetingRoomCreateResponseMessage HandleRequestResponse(VirtualMeetingRoomCreateRequestMessage message)
        {
            _timer = new Stopwatch();
            _timer.Start();
            _serializedRequest = Serialization.DataContractSerialize(message);

            var requestProcessor = new VirtualMeetingRoomCreateProcessor(_logger, _settings);
