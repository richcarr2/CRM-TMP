...
namespace VA.TMP.Integration.VirtualMeetingRoom.PipelineSteps.OnDemand
{
    ...
    /// </summary>
    public class GetIntegrationSettingsStep : IFilter<VmrOnDemandCreateStateObject>
    {
        ...
        public void Execute(VmrOnDemandCreateStateObject state)
        {
            using (var context = new Xrm(state.OrganizationServiceProxy))
            {
                ...
                state.VirtualMeetingRoomPrefix = integrationSettings.First(x => x.Name == "Virtual Meeting Room On Demand Prefix").Value;
