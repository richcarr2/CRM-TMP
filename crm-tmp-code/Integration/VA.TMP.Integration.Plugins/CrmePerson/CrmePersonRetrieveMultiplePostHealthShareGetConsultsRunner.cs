    ...
    public class CrmePersonRetrieveMultiplePostHealthShareGetConsultsRunner : PluginRunner
    {
        ...
        public override void Execute()
        {
            try
            {
                ...
                Logger.WriteDebugMessage($"Provider Station Code: {providerStationCode}");
                Logger.WriteDebugMessage($"Provider Facility Id: {providerFacilityId}");
                ...
                Logger.WriteDebugMessage($"****Provider Consult Count {response.ProviderConsults.Count}");
                Logger.WriteDebugMessage($"****Patient Consult Count {response.PatientConsults.Count}");
                Logger.WriteDebugMessage($"****Provider RTC Count {response.ProviderReturnToClinicOrders.Count}");
                Logger.WriteDebugMessage($"****Patient RTC Count {response.PatientReturnToClinicOrders.Count}");
                ...
                Logger.WriteDebugMessage($"****Entity Count after Distinct is {distinct.Count}");
        ...
        {
            Logger.WriteDebugMessage($"MapHealthShareResponseToCrm Started. patientLoginStationNumber: {patientLoginStationNumber}, providerLoginStationNumber:{providerLoginStationNumber}, isSft: {isSft}");
        ...
        {
            logger.WriteDebugMessage("Starting GetStationNumberFromFacilityId");
