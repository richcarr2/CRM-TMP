                ...
                using (var context = new Xrm(OrganizationService))
                {
                    ...
                    if (!(!serviceAppointment.cvt_Type.Value && serviceAppointment.mcs_groupappointment.Value) && VistaPluginHelpers.RunVvs(serviceAppointment, context, Logger))
                    {
                        ...
                        else
                        {
                            ...
                            //var cernerResponse = CernerHelper.FireLogicApp(PrimaryEntity.ToEntityReference(), context, OrganizationService, Logger);
                            ...
                            //else
                            //{
                            //    var errorMessage = string.Format("Cerner cancel service appointment failure", cernerResponse.ExceptionMessage);
                            //    IntegrationPluginHelpers.CreateIntegrationResultOnCernerFailure("Cerner cancel service appointment failure", errorMessage, cernerResponse.RequestMessage, typeof(TmpCernerOutboundResponseMessage).FullName,
                            //        typeof(TmpCernerOutboundResponseMessage).FullName, MessageRegistry.CernerOutboundResponseMessage, serviceAppointment.Id, OrganizationService,
                            //        cernerResponse?.RequestMessage, cernerResponse.ResponseMessage, 0, 0, cernerResponse.MessageProcessingTime, true, false);

                            //    Success = false;

                            //}

                            //ProcessCernerVideoVisitDeleteResponseMessage(cernerResponse);
                        }
