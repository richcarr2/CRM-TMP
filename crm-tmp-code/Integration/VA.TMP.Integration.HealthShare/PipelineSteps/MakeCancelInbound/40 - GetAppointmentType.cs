...
namespace VA.TMP.Integration.HealthShare.PipelineSteps.MakeCancelInbound
{
    ...
    /// </summary>
    public class GetAppointmentTypeStep : IFilter<MakeCancelInboundStateObject>
    {
        ...
        public void Execute(MakeCancelInboundStateObject state)
        {
            if (state.ServiceAppointment.cvt_Type.HasValue && state.ServiceAppointment.cvt_Type.Value 
                && state.ServiceAppointment.cvt_patientsiteresourcesrequired.HasValue && !state.ServiceAppointment.cvt_patientsiteresourcesrequired.Value)
            {
