...
namespace VA.TMP.Integration.VideoVisit.MessageHandler
{
    ...
    /// Video Visit handler.
    /// </summary>
    public class VideoVisitGetLoanedDevicesHandler
    {
        ...
        public VideoVisitGetLoanedDevicesHandler(ILog logger, Settings settings)
        {
            _logger = logger;
            _settings = settings;
        }
        ...
        public VideoVisitGetLoanedDevicesResponseMessage HandleRequestResponse(VideoVisitGetLoanedDevicesRequestMessage message)
        {
            ...
            var requestProcessor = new VideoVisitGetLoanedDevicesProcessor(_logger, _settings);
            var response = requestProcessor.Execute(message);

            if (response.ExceptionOccured) _logger.Error(string.Format("ERROR: Video Visit Get Loaned Devices: {0}", response.ExceptionMessage));
