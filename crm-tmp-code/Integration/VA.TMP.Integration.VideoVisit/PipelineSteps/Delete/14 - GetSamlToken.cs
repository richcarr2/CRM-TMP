...
namespace VA.TMP.Integration.VideoVisit.PipelineSteps.Delete
{
    ...
    /// Get SAML Token.
    /// </summary>
    public class GetSamlTokenStep : IFilter<VideoVisitDeleteStateObject>
    {
        ...
        public void Execute(VideoVisitDeleteStateObject state)
        {
            using (var srv = new Xrm(state.OrganizationServiceProxy))
            {
                ...
                if (currentUser == null) throw new SamlTokenException(string.Format("Unable to retrieve user {0} for deleting a Video Visit.", state.UserId));
                if (string.IsNullOrEmpty(currentUser.cvt_SAMLToken)) throw new SamlTokenException(string.Format("Unable to retrieve SAML token for user {0} for deleting a Video Visit.", state.UserId));
