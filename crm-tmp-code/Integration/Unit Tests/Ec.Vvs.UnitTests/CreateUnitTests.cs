...
namespace Ec.Vvs.UnitTests
{
    ...
    public class CreateUnitTests
    {
        ...
        public void Initialize()
        {
            ...
            {
                CreateAppointmentappointmentString = (ar, s) => { return GetWriteResults(); }
            };
        ...
        public void HappyPath()
        {
            ...
            using (ShimsContext.Create())
            {
                ...
                var controller = new StubCreateController(_mockLogger, _settings, _mockServiceFactory, _mockTelemetryClient)
                {
                    Request = new StubHttpRequestMessage(),
                    Configuration = new StubHttpConfiguration()
                };

                ShimHttpResponseMessageExtensions.TryGetContentValueOf1HttpResponseMessageM0Out((HttpResponseMessage rm, out EcTmpCreateAppointmentResponse r) =>
                {
                    rm = new HttpResponseMessage();
                    r = new StubEcTmpCreateAppointmentResponse()
                    {
                ...
                var request = new StubEcTmpCreateAppointmentRequest(); //GetRequest();

                // Act
                var httpResponse = controller.Post(request);
                var responseContent = httpResponse.TryGetContentValue(out EcTmpCreateAppointmentResponse response);
                ...
                Assert.AreEqual("Create", response.EcTmpWriteResults.EcTmpWriteResult.First().Reason);
        ...
        public void FailWhenRequestIsNull()
        {
            ...
            using (ShimsContext.Create())
            {
                ...
                ShimHttpResponseMessageExtensions.TryGetContentValueOf1HttpResponseMessageM0Out((HttpResponseMessage rm, out EcTmpCreateAppointmentResponse r) =>
                {
                    ...
                    r = new StubEcTmpCreateAppointmentResponse()
                    {
                        ExceptionMessage = "Error calling VVS Create. The request cannot be null",
                ...
                Assert.AreEqual("Error calling VVS Create. The request cannot be null", response.ExceptionMessage);
            }
        }

        private static StubEcTmpCreateAppointmentRequest GetRequest()
        {
            var request = new StubEcTmpCreateAppointmentRequest
            {
                EcTmpCreateAppointmentRequestDataInfo = new StubEcTmpCreateAppointmentRequestData
                {
            ...
            var patient = new StubEcTmpPatients
            {
            ...
            var provider = new StubEcTmpProviders
            {
                ContactInformation = new EcTmpContactInformation
                {
            ...
            request.EcTmpCreateAppointmentRequestDataInfo.Patients = patients;
            request.EcTmpCreateAppointmentRequestDataInfo.Providers = providers;
